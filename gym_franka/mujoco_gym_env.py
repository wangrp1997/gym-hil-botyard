...
from gym_franka.controllers import opspace
...
class FrankaGymEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        home_position: np.ndarray = np.asarray((0, -0.785, 0, -2.35, 0, 1.57, np.pi / 4)),
        cartesian_bounds: np.ndarray = np.asarray([[0.2, -0.3, 0], [0.6, 0.3, 0.5]]),
    ):

        if xml_path is None:
            xml_path = Path(__file__).parent.parent / "gym_franka" / "assets" / "scene.xml"
